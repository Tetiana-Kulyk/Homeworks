...
@pytest.mark.login
@pytest.mark.smoke
def test_failed_login(expand_dropdown_menu):
    main_page = expand_dropdown_menu.set_email(get_user_creds()[1]).set_password(
        get_user_creds()[1]).click_login_button()
    assert False, 'Account button is not displayed'
